...
class OrderItemService:
    def __init__(self, order_repo:IOrderRepository, order_item_repo:IOrderItemRepository, product_repo:IProductRepository, session:Session):
        ...
        self.order_repo_item=order_item_repo
        self.product_repo=product_repo
        self.session=session
        
    def add_item(self,order_id:int, product_id:int, quantity:int)->OrderItemRead:
        if quantity <1:
        ...
        subtotal=product.sale_price*Decimal(quantity)
        ...
        item=self.order_repo_item.create(item_data)
    ...
    def update_item_quantity(self, item_id:int, new_quantity:int)->OrderItemRead:
        item=self.order_repo_item.get_by_id(item_id)
        if not item:
    ...
    def remove_item(self,item_id:int)->bool:
        ...
        return self.order_repo_item.delete(item_id)
    
    def list_items(self, order_id:int)->List[OrderItemRead]:
        items=self.order_repo_item.get_by_order(order_id)
